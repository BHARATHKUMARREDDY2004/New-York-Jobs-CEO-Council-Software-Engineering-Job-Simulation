...
class Invoice:
    ...
    def calculate_total(self, discount):
        ...
        for item in self.items:
            price = item["price"]
            tax = item["tax"]
            # Initially the calculation of total amount equation is wrong equation to calculate

            # total += price + price * tax * discount

            # correct equation 
            item_total_price = price + ( price * tax )                 # Item total price (actual price + tax on item )
            total_discount = item_total_price * ( discount / 100 )     # Discount applied to item total price
            total += item_total_price - total_discount                 # Total amount to pay after excluding discount from total price
        return total
